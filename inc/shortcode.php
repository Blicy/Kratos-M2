...
function hide($atts,$content=null,$code=""){
    ...
    if($reply_to_this=='true'){
        ...
        if(!$comments) $content = '<div class="hide_notice"><i class="fa fa-lock"></i> 内容藏起来了呢，<a href="'.wp_login_url(get_permalink()).'" rel="nofollow">登录</a>并在本文发表评论之后再找找看哟~</div>';
    }else{
        if($email){
            global $wpdb;
            global $id;
        ...
        if(!$comments) $content = '<div class="hide_notice"><i class="fa fa-lock"></i> 内容藏起来了呢，<a href="'.wp_login_url(get_permalink()).'" rel="nofollow">登录</a>并在本站任一文章发表评论之后再找找看哟~</div>';
    }
    if($comments) $content = '<div class="unhide"><div class="info"><i class="fa fa-lock-open"></i>被你找到了呢：</div>'.$content.'</div>';
    return $content;
}
