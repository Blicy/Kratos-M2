...
// ——————————————————————— AJAX-评论、搜索、分页 等
var ajx_main = '#container' , // 要替换的主体id，改为你文章部分的容器
ajx_a = 'a[target!=_blank]' , // a标签，自己添加排除规则
ajx_comt = 'comments' , // 整个评论区的id ,不加#
ajx_submit_form = '#commentform' , // 提交按钮所在的表单
ajx_comtlist = '.comment-list' , // 评论列表id或class
ajx_comtpagenav = '#comments-nav' , // 评论分页导航的id或class
...
function ajax(reqUrl, msg, method, data) {
    if (msg == 'pagelink' || msg == 'search') { // 页面、搜索
        $(ajx_main).fadeTo(700,0.3); 
    } else if ( msg == 'comment' ){ // 评论提交    
        $('#' + ajx_comt).fadeTo(700,0.3); 
    } else if ( msg == 'comtpagenav' ){ //  评论分页时
        $(ajx_comtlist).fadeTo(700,0.3);
        $(ajx_comtpagenav).fadeTo(700,0.3);
    }
    $.ajax({
        url: reqUrl, 
        type: method,
        ...
        success: function(data) {
            ...
            if (msg == 'pagelink' || msg == 'search') { // 又如果msg为 页面 或 搜索—— 【1】
                ...
                $(ajx_comtlist).fadeTo(700,1); // 评论列表显示
                $(ajx_comtpagenav).fadeTo(700,1); // 评论分页显示
        ...
        error: function(request) { // 错误时的处理
            if (msg == 'pagelink' || msg == 'search'){
                ...
                $(ajx_comtlist).fadeTo(700,1); // 警告后评论区显示
                $(ajx_comtpagenav).fadeTo(700,1); // 警告后评论区显示
            } else {
